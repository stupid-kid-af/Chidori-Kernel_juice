...
//    int inter_words_delay;
    ...
    if(ret < 0){
        NVT_ERR("read spi-cs-clk-delay failed!(%d)\n", ret);
        return ret;
    }
#endif
    NVT_ERR("cs_clk_delay = %d\n", cs_clk_delay);
    ...
    if (delay_params == NULL) {
        NVT_ERR("failed to allocated memory for delay_params\n");
        return -ENOMEM;
    }
...
static int32_t nvt_ts_probe(struct spi_device *client)
{
	...
	//strlcpy(ts->lockdowninfo, tp_lockdown_info, 17);
	...
	NVT_LOG("chenwenmin  \n");
	if (active_panel) {
		//NVT_ERR("---active_panel=%s", active_panel);
	...
	bTouchIsAwake = 1;
 	...
 	//---config regulator---
   	ret = nvt_ts_get_regulator(true);
    	if (ret < 0) {
        	NVT_LOG("Failed to get register");
...
*/
	if (!evdata) {
		NVT_LOG("chenwenmin  invalid evdata=%p\n", evdata);
		return 0;
	}
	...
	if (evdata->data && ts) {
		...
		if (event == DRM_PANEL_EARLY_EVENT_BLANK) {
			NVT_LOG("chenwenmin  DRM_PANEL_EARLY_EVENT_BLANK=%d\n", DRM_PANEL_EARLY_EVENT_BLANK);
			if (*blank == DRM_PANEL_BLANK_POWERDOWN) {
				NVT_LOG("chenwenmin  DRM_PANEL_BLANK_POWERDOWN=%d\n", DRM_PANEL_BLANK_POWERDOWN);
				NVT_LOG("chenwenmin tp suspend event=%lu, *blank=%d\n", event, *blank);
				flush_workqueue(ts->event_wq);
				nvt_ts_suspend(&ts->client->dev);
			}
		} else if (event == DRM_PANEL_EVENT_BLANK) {
			NVT_LOG("chenwenmin  DRM_PANEL_EVENT_BLANK=%d\n", DRM_PANEL_EVENT_BLANK);
			if (*blank == DRM_PANEL_BLANK_UNBLANK) {
				NVT_LOG("chenwenmin  DRM_PANEL_BLANK_UNBLANK=%d\n", DRM_PANEL_BLANK_UNBLANK);
				NVT_LOG("chenwenmin tp resume event=%lu, *blank=%d\n", event, *blank);
				//nvt_ts_resume(&ts->client->dev);
				flush_workqueue(ts->event_wq);
...
__setup("lockDownInfo", is_lockdown_info_detect);

static bool charger_boot_mode = false;
